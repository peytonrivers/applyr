"""
pipeline_supply_chain.py
JobSpy pipeline: entry-level supply chain jobs in North Carolina
...
QUERIES = [
    ...
    "entry level operations associate",
    ...
    "entry level demand planning analyst",
    "demand planning intern",
    "supply planner",
    "entry level supply planner",
    "junior supply planner",
    "planning analyst",
    "entry level planning analyst",
    "junior planning analyst",
    "s&op analyst",
    "entry level s&op analyst",
    "sales and operations planning analyst",
    "entry level sales and operations planning analyst",

    # -------------------------
    # INVENTORY
    # -------------------------
    "inventory analyst",
    "entry level inventory analyst",
    "junior inventory analyst",
    "inventory coordinator",
    "entry level inventory coordinator",
    "inventory control analyst",
    "entry level inventory control analyst",
    "inventory planner",
    "entry level inventory planner",
    "materials analyst",
    "entry level materials analyst",
    ...
    "junior erp analyst",
    "sap analyst",
    "entry level sap analyst",
    "junior sap analyst",
    "supply chain systems analyst",
    "entry level supply chain systems analyst",

    # -------------------------
    # PROCESS IMPROVEMENT
    # -------------------------
    "process improvement analyst",
    "entry level process improvement analyst",
    "junior process improvement analyst",
    "continuous improvement analyst",
    "entry level continuous improvement analyst",
    "lean analyst",
    "entry level lean analyst",
    "six sigma analyst",
    "entry level six sigma analyst",

    # -------------------------
    # GENERAL VARIANTS
    # -------------------------
    "supply chain data analyst",
    "entry level supply chain data analyst",
    "operations data analyst",
    "entry level operations data analyst",
    "supply chain reporting analyst",
    "entry level supply chain reporting analyst",
]
...
ROLE_KEYWORDS = [
    "supply chain analyst", "supply chain associate", "supply chain coordinator",
    "supply chain specialist", "supply chain intern", "supply chain data",
    "supply chain systems", "supply chain reporting",
    "operations analyst", "operations coordinator", "operations associate",
    "business operations analyst",
    "demand planner", "demand planning analyst", "supply planner",
    "planning analyst", "s&op analyst", "sales and operations planning",
    "inventory analyst", "inventory coordinator", "inventory control",
    "inventory planner", "materials analyst",
    "erp analyst", "sap analyst",
    "process improvement analyst", "continuous improvement analyst",
    "lean analyst", "six sigma analyst",
]

# Titles that superficially match ROLE_KEYWORDS but are NOT supply chain roles.
NON_ROLE_TITLES = [
    "sales coordinator",        # sales, not supply chain
    ...
    "insurance analyst",        # insurance
    "real estate",              # real estate
    "construction",             # construction
    "manufacturing engineer",   # engineering
    "industrial engineer",      # engineering
    "warehouse associate",      # physical/hourly role
    "forklift",                 # physical/hourly role
    "driver",                   # physical/hourly role
]
...
GENERIC_TITLE_WORDS = {
    ...
    "level", "supply", "chain", "operations", "planning", "open",
}
...
def is_target_role(title: str) -> bool:
    """
    Returns True if the job title is a genuine supply chain role.

    Two-step check:
      1. The title must contain at least one word from ROLE_KEYWORDS
         (e.g. "supply chain analyst", "demand planner", "erp analyst").
      2. The title must NOT contain any phrase from NON_ROLE_TITLES —
         these are titles that superficially match supply chain keywords
         but belong to a different field (e.g. "warehouse associate"
         matches "associate" but is a physical/hourly role).
...
def run():
    ...
    Flow for each scraped job:
      ...
      3.  Role check — skip non-supply chain titles
        ...
        for query in QUERIES:
            ...
            for _, row in df.iterrows():
                ...
                if not is_target_role(title):
                    print(f"  ⏭️  Not a supply chain role")
                    skipped_role += 1
                    continue
