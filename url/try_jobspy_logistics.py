"""
pipeline_logistics_procurement.py
JobSpy pipeline: entry-level logistics and procurement jobs in North Carolina
...
QUERIES = [
    ...
    "logistics associate",
    "entry level logistics associate",
    "logistics intern",
    ...
    "entry level transportation analyst",
    "junior transportation analyst",
    "transportation coordinator",
    "entry level transportation coordinator",
    "junior transportation coordinator",
    "freight analyst",
    "entry level freight analyst",
    "junior freight analyst",
    "freight coordinator",
    "entry level freight coordinator",
    "carrier analyst",
    "entry level carrier analyst",
    "import export coordinator",
    "entry level import export coordinator",
    "trade compliance analyst",
    "entry level trade compliance analyst",
    "customs analyst",
    "entry level customs analyst",

    # -------------------------
    # 3PL / DISTRIBUTION
    # -------------------------
    "3pl coordinator",
    "entry level 3pl coordinator",
    "distribution coordinator",
    "entry level distribution coordinator",
    "distribution analyst",
    "entry level distribution analyst",
    "fulfillment coordinator",
    "entry level fulfillment coordinator",
    "fulfillment analyst",
    "entry level fulfillment analyst",
    "shipping coordinator",
    "entry level shipping coordinator",
    "receiving coordinator",
    "entry level receiving coordinator",
    ...
    "entry level procurement associate",
    "procurement coordinator",
    "entry level procurement coordinator",
    "junior procurement coordinator",
    "procurement specialist",
    "entry level procurement specialist",
    "procurement graduate",
    "new grad procurement",

    # -------------------------
    # PURCHASING
    # -------------------------
    "purchasing analyst",
    "entry level purchasing analyst",
    ...
    "junior buyer",
    "assistant buyer",
    "entry level assistant buyer",

    # -------------------------
    # SOURCING
    # -------------------------
    "sourcing analyst",
    "entry level sourcing analyst",
    "junior sourcing analyst",
    "sourcing coordinator",
    "entry level sourcing coordinator",
    "sourcing associate",
    "entry level sourcing associate",
    "strategic sourcing analyst",
    "entry level strategic sourcing analyst",
    "global sourcing analyst",
    "entry level global sourcing analyst",

    # -------------------------
    # VENDOR / CONTRACTS
    # -------------------------
    "vendor analyst",
    "entry level vendor analyst",
    "junior vendor analyst",
    "vendor coordinator",
    "entry level vendor coordinator",
    "contracts analyst",
    "entry level contracts analyst",
    "junior contracts analyst",
    "contract administrator",
    "entry level contract administrator",
    "supplier analyst",
    "entry level supplier analyst",
...
ROLE_KEYWORDS = [
    # Logistics
    "logistics coordinator", "logistics analyst", "logistics associate",
    "logistics specialist", "logistics assistant", "logistics intern",
    "transportation analyst", "transportation coordinator",
    "freight analyst", "freight coordinator",
    "carrier analyst", "import export coordinator",
    "trade compliance analyst", "customs analyst",
    "3pl coordinator", "distribution coordinator", "distribution analyst",
    "fulfillment coordinator", "fulfillment analyst",
    "shipping coordinator", "receiving coordinator",
    # Procurement
    "procurement analyst", "procurement associate", "procurement coordinator",
    "procurement specialist", "procurement intern",
    "purchasing analyst", "purchasing coordinator", "purchasing associate",
    "buyer", "assistant buyer",
    "sourcing analyst", "sourcing coordinator", "sourcing associate",
    "strategic sourcing", "global sourcing",
    "vendor analyst", "vendor coordinator",
    "contracts analyst", "contract administrator",
    "supplier analyst",
]

# Titles that superficially match ROLE_KEYWORDS but are NOT logistics/procurement roles.
NON_ROLE_TITLES = [
    "sales coordinator",        # sales, not logistics
    ...
    "sourcing specialist",      # HR recruiting / staffing
    "staffing coordinator",     # staffing firms
    "driver",                   # physical/hourly
    "forklift",                 # physical/hourly
    "warehouse associate",      # physical/hourly
    "delivery driver",          # physical/hourly
    "truck driver",             # physical/hourly
]
...
GENERIC_TITLE_WORDS = {
    ...
    "level", "logistics", "procurement", "sourcing", "purchasing", "open",
}
...
def is_target_role(title: str) -> bool:
    """
    Returns True if the job title is a genuine logistics or procurement role.

    Two-step check:
      1. The title must contain at least one word from ROLE_KEYWORDS
         (e.g. "logistics coordinator", "procurement analyst", "buyer").
      2. The title must NOT contain any phrase from NON_ROLE_TITLES —
         these are titles that superficially match keywords but belong
         to a different field (e.g. "driver" matches but is physical/hourly,
         "talent acquisition sourcing" matches "sourcing" but is HR).
...
def run():
    ...
    Flow for each scraped job:
      ...
      3.  Role check — skip non-logistics/procurement titles
        ...
        for query in QUERIES:
            ...
            for _, row in df.iterrows():
                ...
                if not is_target_role(title):
                    print(f"  ⏭️  Not a logistics or procurement role")
                    skipped_role += 1
                    continue
