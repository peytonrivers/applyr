...
URL Acceptance Architecture (whitelist-first):
  ...
  URLs that pass the gate then go through SECONDARY FILTERS:
    → Blocked extensions, blocked domains, aggregator check,
      generic URL endings, HTTP liveness check (HEAD + GET signals)
...
# ─── Secondary Filter: Aggregators ───────────────────────────────────────────
# Belt-and-suspenders against any aggregator that somehow passes the
# acceptance gate (e.g. a company named "Indeed" or short token collision).
...
def passes_acceptance_gate(url: str, company: str) -> tuple[bool, str]:
    ...
    Acceptance criteria (either condition is sufficient):
      1. URL domain is a known ATS platform
      2. A company name token appears in the URL domain
    """
    if not url:
        return False, "Empty URL"

    if is_ats_url(url):
        return True, f"Known ATS domain"
...
def passes_secondary_filters(url: str) -> tuple[bool, str]:
    ...
    if has_blocked_extension(url):
        return False, f"Blocked file extension"
    ...
    if is_generic_url(url):
        return False, f"Generic careers page — not a specific listing"
...
def is_valid_location(loc: str) -> bool:
    """Returns True if the location string is within the target metro area."""
...
def full_url_check(url: str, company: str, label: str) -> str | None:
    ...
    Runs the complete validation pipeline on a single URL:
      ...
      2. Acceptance gate  (ATS domain OR company name in domain)
      3. Secondary filters (extensions, blocked domains, aggregators, generic endings)
