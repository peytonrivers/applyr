...
AGGREGATORS = [
    ...
    # ── Newly caught aggregators ──────────────────────────────────────────────
    "jobs.chronicle.com",       # Chronicle of Higher Ed board
    "tallo.com",                # student/early career aggregator
    "jobleads.com",             # aggregator
    "entertainmentcareers.net", # niche aggregator
    "jobilize.com",             # aggregator
    "myjobhelper.com",          # aggregator
    "jobs.accaglobal.com",      # ACCA professional body board
    "talentbridge.com",         # staffing firm
    "accruepartners.com",       # staffing firm
    # ── Other staffing firms / non-direct employers ───────────────────────────
    "roberthalf.com", "adecco.com", "manpower.com", "randstad.com",
    "kforce.com", "apexgroup.com", "staffmark.com", "aerotek.com",
    "heidrick.com", "michaelpage.com", "spencerstuart.com",
]

# ─── Obviously Invalid URL Signals ───────────────────────────────────────────

BLOCKED_EXTENSIONS = {".pdf", ".doc", ".docx", ".png", ".jpg", ".jpeg", ".xls", ".xlsx"}

# Domains that will never host a job listing
BLOCKED_DOMAINS = {
    "issuu.com",            # document hosting
    "scribd.com",           # document hosting
    "docs.google.com",      # Google Docs
    "drive.google.com",     # Google Drive
    "dropbox.com",          # file storage
    "facebook.com",         # social
    "twitter.com",          # social
    "x.com",                # social
    ...
    "medium.com",           # blog
    "substack.com",         # blog
    "ijglobal.com",         # financial media
    "jamaicabobsleigh.org", # caught in the wild
}
...
def is_obviously_invalid(url: str) -> tuple[bool, str]:
    """
    Fast pre-flight check before any HTTP calls are made.
    Catches documents, blocked domains, and clearly non-job URLs.
    Returns (True, reason) if the URL should be rejected, (False, "") otherwise.
    """
    if not url:
    ...
    # Reject document file extensions
    if any(path.endswith(ext) for ext in BLOCKED_EXTENSIONS):
        return True, f"Document file extension in path: {path}"

    # Reject known non-job domains
    if domain in BLOCKED_DOMAINS:
        return True, f"Blocked domain: {domain}"

    # Reject aggregators (redundant safety net — aggregators are also
    # caught later, but catching here avoids a Serper lookup entirely)
...
def is_generic_url(url: str) -> bool:
    ...
    if len(segments) <= 1 and not parsed.query:
        # Don't flag known ATS domains that use single-segment company paths
        if parsed.netloc not in {"jobs.lever.co", "boards.greenhouse.io", "job-boards.greenhouse.io"}:
            return True
    return False
...
def check_url_quality(url: str) -> dict:
    ...
    Steps:
      0. Reject obviously invalid URLs immediately (no HTTP call).
    ...
    """
    # Step 0: Obviously invalid (documents, blocked domains, aggregators)
    invalid, reason = is_obviously_invalid(url)
    if invalid:
        return {"is_active": False, "reason": f"Obviously invalid — {reason}"}

    # Step 1: Generic career homepage check
...
def find_apply_url(title: str, company: str, location: str) -> str | None:
    ...
    Returns the first result that passes the obviously-invalid check
    from the top 10, or None.
    Does NOT run the full quality check here — let resolve_and_validate handle that.
    """
    ...
    try:
        ...
        for result in results[:10]:
            ...
            # Pre-screen Serper results before spending an HTTP call on them
            invalid, reason = is_obviously_invalid(link)
            if invalid:
                print(f"     ⛔ Skipping result — {reason}")
                continue

            if is_aggregator(link):
                print(f"     ⛔ Skipping aggregator result — {link}")
                continue
