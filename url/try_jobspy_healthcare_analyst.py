"""
pipeline_healthcare_analyst.py
JobSpy pipeline: entry-level healthcare analyst jobs in North Carolina
...
QUERIES = [
    ...
    "entry level healthcare operations analyst",
    "junior healthcare operations analyst",
    "healthcare business analyst",
    "entry level healthcare business analyst",
    "junior healthcare business analyst",
    "healthcare reporting analyst",
    "entry level healthcare reporting analyst",
    "healthcare graduate",
    "new grad healthcare analyst",

    # -------------------------
    # HEALTH INFORMATICS
    # -------------------------
    "health informatics analyst",
    "entry level health informatics analyst",
    "junior health informatics analyst",
    "health informatics intern",
    "health informatics associate",
    "entry level health informatics associate",
    "clinical informatics analyst",
    "entry level clinical informatics analyst",
    "junior clinical informatics analyst",
    "medical informatics analyst",
    "entry level medical informatics analyst",
    "nursing informatics analyst",
    "entry level nursing informatics analyst",

    # -------------------------
    # CLINICAL DATA
    # -------------------------
    "clinical data analyst",
    "entry level clinical data analyst",
    "junior clinical data analyst",
    "clinical data coordinator",
    "entry level clinical data coordinator",
    "clinical data associate",
    "entry level clinical data associate",
    "clinical data intern",
    "clinical research analyst",
    "entry level clinical research analyst",
    "junior clinical research analyst",
    ...
    "entry level population health analyst",
    "junior population health analyst",
    "population health intern",
    "population health associate",
    "entry level population health associate",
    "value based care analyst",
    "entry level value based care analyst",
    "quality improvement analyst",
    "entry level quality improvement analyst",
    "junior quality improvement analyst",

    # -------------------------
    # REVENUE CYCLE
    # -------------------------
    "revenue cycle analyst",
    "entry level revenue cycle analyst",
    "junior revenue cycle analyst",
    "revenue cycle coordinator",
    "entry level revenue cycle coordinator",
    "medical billing analyst",
    "entry level medical billing analyst",
    "medical coding analyst",
    "entry level medical coding analyst",
    "claims analyst",
    "entry level claims analyst",
    "junior claims analyst",
    "denial management analyst",
    "entry level denial management analyst",
    ...
    "entry level patient services coordinator",
    "health services coordinator",
    "entry level health services coordinator",
    "healthcare operations coordinator",
    "entry level healthcare operations coordinator",
    "clinical operations analyst",
    "entry level clinical operations analyst",

    # -------------------------
    # PHARMACY / MANAGED CARE
    # -------------------------
    "pharmacy analyst",
    "entry level pharmacy analyst",
    "junior pharmacy analyst",
    "managed care analyst",
    "entry level managed care analyst",
    "junior managed care analyst",
    "utilization management analyst",
    "entry level utilization management analyst",
    "prior authorization analyst",
    "entry level prior authorization analyst",
    ...
    "entry level medical data analyst",
    "health analytics intern",
    "healthcare analytics analyst",
    "entry level healthcare analytics analyst",
]
...
ROLE_KEYWORDS = [
    # Core
    "healthcare analyst", "healthcare data analyst", "healthcare operations analyst",
    "healthcare business analyst", "healthcare reporting analyst",
    # Informatics
    "health informatics analyst", "clinical informatics analyst",
    "medical informatics analyst", "nursing informatics analyst",
    # Clinical data
    "clinical data analyst", "clinical data coordinator", "clinical data associate",
    "clinical research analyst", "clinical research coordinator",
    "clinical research associate",
    # Population health
    "population health analyst", "population health associate",
    "value based care analyst", "quality improvement analyst",
    # Revenue cycle
    "revenue cycle analyst", "revenue cycle coordinator",
    "medical billing analyst", "medical coding analyst",
    "claims analyst", "denial management analyst",
    # Administration
    "healthcare administrator", "hospital administrator",
    "healthcare coordinator", "patient services coordinator",
    "health services coordinator", "healthcare operations coordinator",
    "clinical operations analyst",
    # Pharmacy / managed care
    "pharmacy analyst", "managed care analyst",
    "utilization management analyst", "prior authorization analyst",
    # General
    "health data analyst", "medical data analyst",
    "health analytics", "healthcare analytics",
    "health informatics intern", "healthcare analyst intern",
]

# Titles that superficially match ROLE_KEYWORDS but are NOT healthcare analyst roles.
NON_ROLE_TITLES = [
    # Clinical / licensed roles — require licensure
    "registered nurse",         # requires RN license
    "licensed practical nurse", # requires LPN license
    "nurse practitioner",       # requires NP license
    "physician assistant",      # requires PA license
    "physical therapist",       # requires PT license
    "occupational therapist",   # requires OT license
    "pharmacist",               # requires PharmD
    "pharmacy technician",      # clinical, not analyst
    "medical assistant",        # clinical
    "dental assistant",         # clinical
    "radiologist",              # requires MD
    "lab technician",           # clinical lab
    "medical technologist",     # clinical lab
    "respiratory therapist",    # requires license
    "surgical technologist",    # clinical
    # Non-analyst healthcare roles
    "patient care technician",  # clinical/bedside
    "home health aide",         # caregiving
    "nursing assistant",        # caregiving
    "caregiver",                # caregiving
    "medical receptionist",     # admin/front desk
    # Other fields using similar keywords
...
GENERIC_TITLE_WORDS = {
    ...
    "level", "healthcare", "health", "clinical", "medical", "open",
}
...
def is_target_role(title: str) -> bool:
    """
    Returns True if the job title is a genuine healthcare analyst role.

    Two-step check:
      1. The title must contain at least one word from ROLE_KEYWORDS
         (e.g. "healthcare analyst", "clinical data analyst",
         "health informatics analyst").
      2. The title must NOT contain any phrase from NON_ROLE_TITLES —
         these are clinical/licensed roles that require licensure
         (RN, LPN, PharmD, PT) or non-analyst healthcare roles like
         caregiver, medical assistant, or patient care technician.
...
def run():
    ...
    Flow for each scraped job:
      ...
      3.  Role check — skip non-healthcare analyst titles
        ...
        for query in QUERIES:
            ...
            for _, row in df.iterrows():
                ...
                if not is_target_role(title):
                    print(f"  ⏭️  Not a healthcare analyst role")
                    skipped_role += 1
                    continue
