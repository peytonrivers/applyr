...
def save_cost_report(state, file_path="cost_report.json"):
    setup_cost_tracker(state)
    with open(file_path, "w", encoding="utf-8") as f:
        json.dump(state["cost_tracker"], f, indent=2)
    print(f"Saved cost report to {file_path}")
...
def new_apply_process(state: ApplicationState):
    ...
    prompt = f"""
Your an AI Applicant Helper and your job is to look at the screenshot and to determine what type of page this is.
Literal["apply", "signup", "forms", "cookies", "other", "error"]
1. apply
2. signup
3. forms
4. cookies
...
"""
    ...
    response = invoke_and_track(apply_process_llm, [
        {
            "role": "user",
            "content": [
...
"""
    ...
    decision = invoke_and_track(structured_llm, prompt, state, "front_page_decision")
...
def new_decide_page(state:ApplicationState):
    ...
    # 2. Extract the final width and height after the page has fully expanded
    full_page_dimensions = page.evaluate("""() => {
        return {
...
1. apply - You choose this page if this is the opening page where we need to click apply now or start the application process.
2. signup - You choose this page if we are required to sign up or login before continuing or if we have to create an account.
    ...
    response = invoke_and_track(decide_page_llm, [
        {
            ...
            "content": [
                {"type": "text", "text": prompt},
                {
                    "type": "image_url",
                    "image_url": {
                        "url": f"data:image/png;base64,{screenshot_bytes}"
                    }
                }
            ]
        }
    ], state, "new_decide_page")
...
def ai_all_elements(state: ApplicationState):
    ...
    response = invoke_and_track(all_elements_llm, prompt, state, "ai_all_elements")
    print(response)
...
"""
    ...
    response = invoke_and_track(multiple_question_llm, prompt, state, "multiple_question")
...
Ex:
 ...
 body_text: {json.dumps(body_text)}
"""
    response = invoke_and_track(signup_process_llm, prompt, state, "signup_process")
...
def new_cookies_process(state: ApplicationState):
    page = state["current_page"]["page"]
    full_page_height = page.evaluate("""() => { return Math.max( document.body.scrollHeight, document.documentElement.scrollHeight, document.body.offsetHeight, document.documentElement.offsetHeight, document.body.clientHeight, document.documentElement.clientHeight ); }""")
    page.set_viewport_size({"width": 1280, "height": full_page_height})
    screenshot = page.screenshot()
...
"""
    ...
    response = invoke_and_track(cookies_process_llm, prompt, state, "cookies_process")
...
body_text: {json.dumps(body_text)}
"""
    response = invoke_and_track(forms_action_llm, prompt, state, "forms_action")
...
"""
    ...
    decision = invoke_and_track(page_decision_llm, prompt, state, "ai_page_decision")
...
def complete_application_forms(state):
    ...
    print_final_cost_summary(state)

    return state

def get_all_body(state: ApplicationState):
    page = state["current_page"]["page"]
...
""")
    ...
    state["body_elements"] = body_elements
    return state
...
graph.add_node("new_decide_page", new_decide_page)
graph.add_node("apply_process", apply_process)
graph.add_node("cookies_process", cookies_process)
graph.add_node("new_cookies_process", new_cookies_process)
